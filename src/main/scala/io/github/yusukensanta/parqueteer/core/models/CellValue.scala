...
/**
 * A typed cell from a Parquet row. Each variant maps to a Parquet
 * logical/primitive type and carries the decoded JVM value.
 */
enum CellValue:
  case Null
  case Str(s: String)
...
object CellValue:
  ...
  extension (v: CellValue)
    ...
    def display: String = v match
      ...
      case F32(f) =>
        ...
        else new java.math.BigDecimal(f.toString).stripTrailingZeros.toPlainString
      case F64(d) =>
  ...
  def sanitizeTerminal(s: String): String =
    if s.exists(isControlCode) then s.filterNot(isControlCode) else s
