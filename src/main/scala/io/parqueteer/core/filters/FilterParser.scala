package io.parqueteer.core.filters
 ...
 *
 * Supported Syntax:
 * - Comparisons: column = value, column > value, column >= value, column < value, column <= value
...
class FilterParser extends JavaTokenParsers {
  ...
  def parseFilter(input: String): Try[Filter] = {
    parseAll(expression, input) match {
      ...
      case NoSuccess(msg, _) => TryFailure(
  ...
  private def andExpression: Parser[Filter] = {
    notExpression ~ rep("AND" ~> notExpression) ^^ {
      case first ~ rest =>
        rest.foldLeft(first) { (acc, filter) =>
          acc && filter
        }
  ...
  private def comparison: Parser[Filter] = {
    columnName ~ operator ~ value ^^ {
      case col ~ op ~ v => buildComparison(col, op, v)
    }
  }
   ...
   */
  private def buildComparison(column: String, operator: String, value: Any): Filter = {
    val col = Col(column)
